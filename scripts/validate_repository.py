...
EXPECTED_COUNT=402
BOOK_HASH='14eff00a111e9e985cc86e5e640cd9ae967f2e170e7d0768e765f80aa1c092d5'
PROOF_HASH='35bca9ce3809cc55a82bdd6e82548cf292458d32792aa10d3066d2aa3da1fa6a'
FIG_HASH='3c1ea05af05ce28d50195a86b27327c933a6fd719bf76158a60014e6834091ec'
ARCHIVE_HASH='3611fd2e2c445ece5804094cb8271b0a02322a238bdaf9b0866560c034165700'
ZENODO_VERSION_DOI='10.5281/zenodo.22017362'
ZENODO_CONCEPT_DOI='10.5281/zenodo.22017361'
GITHUB_RELEASE='https://github.com/antonioclim/informatics-students-guide-companion/releases/tag/v1.2.0'
errors=[]
def req(cond,msg):
...
req(ARCHIVE_HASH in version,'archived payload hash missing')
req(ZENODO_VERSION_DOI in version,'version DOI missing from VERSION.txt')
req(ZENODO_CONCEPT_DOI in version,'all-versions DOI missing from VERSION.txt')
...
req(ZENODO_VERSION_DOI in active,'verified Zenodo version DOI missing')
req(ZENODO_CONCEPT_DOI in active,'verified Zenodo all-versions DOI missing')
req(ARCHIVE_HASH in active,'verified archive hash missing')
found=set(re.findall(r'10\.5281/zenodo\.\d+',active))
allowed={ZENODO_VERSION_DOI,ZENODO_CONCEPT_DOI}
req(found<=allowed,'unexpected Zenodo DOI in active metadata: '+str(sorted(found-allowed)))
if yaml:
 try:
  ...
  req(cff.get('doi')==ZENODO_VERSION_DOI,'CFF DOI mismatch')
  req(str(cff.get('date-released'))=='2026-08-24','CFF release date mismatch')
  req(cff.get('url')=='https://doi.org/'+ZENODO_VERSION_DOI,'CFF URL mismatch')
  req('license' not in cff,'CFF must not collapse the mixed licence architecture into one licence')
...
req('doi' not in z and 'prereserve_doi' not in z,'deposit-template metadata must not self-assert DOI control fields')
rels=z.get('related_identifiers') or []
req(any(r.get('identifier')==GITHUB_RELEASE and r.get('relation')=='isIdenticalTo' and r.get('scheme')=='url' for r in rels),'Zenodo metadata does not identify the GitHub v1.2.0 release')
for p in files:
...
print(json.dumps({'status':'FAIL' if errors else 'PASS','file_count':len(files),'figure_counts':counts,'zenodo_version_doi':ZENODO_VERSION_DOI,'zenodo_all_versions_doi':ZENODO_CONCEPT_DOI,'errors':errors},ensure_ascii=False,indent=2)); sys.exit(1 if errors else 0)
